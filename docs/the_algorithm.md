...
1. Retrieve all papers that cite it.
2. For each citing paper, check whether its publication date is within 18 months after the target paper's publication date.
3. Count only those citations.
...
Any paper with total citations < m can be safely excluded because:
...
1. Retrieve its citing papers.
2. Count citations whose publication dates fall within the first 18 months after publication.
...
In most scientific fields, the number of papers satisfying total citations >= m will be much smaller than the total number of papers, reducing computation by orders of magnitude while preserving exact results.
...
When this algorithm is used together with the LLM topic filter, there are two independent multipliers:

1. **LLM pool multiplier** (`--initial-multiplier` / `--max-multiplier`): how many exact top 18m-cited papers to send to the LLM relevance filter before returning the final top N on-topic papers.
2. **Citation initial multiplier** (`--citation-initial-multiplier`): the kN multiplier described in this algorithm, used internally to compute the exact top 18m-cited papers for the requested pool size.

For example, if the final target is 50 papers and the LLM pool multiplier is 1.5, the tool asks this algorithm for the exact top 75 papers by 18m citations. The citation initial multiplier then controls this algorithm's initial set size S for that 75-paper request. The default citation initial multiplier is 1.5, while the illustrative default in the algorithm description above uses 2N as the general case.
